...
describe('PacienteMedicoService', () => {
  ...
  it('addMedicoPaciente should add an medico to a paciente', async () => {
    ...
    expect(result.medicos[0].name).toBe(newMedico.name)
  ...
  it('addMedicoPaciente should thrown exception for an invalid medico', async () => {
    ...
    await expect(() => service.addMedicoPaciente(newPaciente.id, "0")).rejects.toHaveProperty("message", "The medico with the given id was not found");
  });
